...
module EFTCAMB_Kmouflage_Mod
    ...
    !----------------------------------------------------------------------------------------
    !> This is the type that contains the definition of K-mouflage models.
...
contains
    ...
    contains
        ...
        subroutine derivs( num_eq, x, y, ydot )
            ...
            HdotL = -1.5_dl*a**2 * params_cache%h0_Mpc**2 * (Omegam_EFT/a**3 + 4._dl*Omegarad_EFT/(3._dl*a**4)) + adotoaL**2
                ...
                vfunc= -Av**2 *(1._dl-eps2)*Omegavac_EFT+Av**2 *(1-eps2)*Omegarad_EFT/(3._dl*a**4) - Av**4 * Omegarad_EFT/(3.*a**4)+Av**2 *(1-eps2) &
          ...
          !      d2Kdchi22= - ((Av**4) * Ap / Mnorm + 6._dl* a**2 * Av* chi* dKdchi - 6._dl* a**3 * chi* Ap* dKdchi + a**3 * Av &
            !        & * chip* dKdchi)/(2._dl * a**3 * Av* chi* chip)

      !          d3Kdchi33= (-3._dl* a* Av**4 * Ap**2 *chi *chip / Mnorm - 6._dl* a**4 *chi**2 *Ap**2 * chip * dKdchi &
       !             & + Av**5 *(chip* (Ap* (3._dl* chi + a* chip) -a* chi* App) +  a* chi* Ap* chipp)/Mnorm + 6._dl &
        !            & * a**4 * Av* chi**2 *(dKdchi *(chip * App- Ap* chipp) + Ap*chip**2 * d2Kdchi2)+ a**2 * Av**2 * (dKdchi &
         !           & * (a**2 * chip**3 + 6._dl *chi**2 *(chip +a*chipp))-a*chi* chip**2 *(6._dl *chi+a*chip) &
          !          & * d2Kdchi2))/(2._dl* a**4 * Av**2 *chi**2 *chip**3)  
                ...
                HdotL = -1.5_dl*a**2 * params_cache%h0_Mpc**2 * (Omegam_L/a**3 + 4._dl*Omegarad_EFT/(3._dl*a**4)) + adotoaL**2
        ...
        subroutine output( num_eq, ind, x, y )
            ...
            if ( DebugEFTCAMB ) then
                ...
                if ( is_open ) then
                    write (33,'(20E15.5)') a, adotoa, (adotoa- adotoaL)/adotoaL, (1.+eps1)*Av**2, OmegaPhi, p_phi/rho_phi
                end if
    ...
    function EFTCAMBKmouflageAdditionalModelStability( self, a, eft_par_cache, eft_cache )
        ...
        m_lim=0._dl
        !m_lim= (4._dl + 3._dl*(eft_par_cache%omegab + eft_par_cache%omegac) + 4._dl*(eft_par_cache%omegag + eft_par_cache%omegar) + self%gammaA *(2._dl + 3._dl*(eft_par_cache%omegab + eft_par_cache%omegac)+ 4._dl*(eft_par_cache%omegag + eft_par_cache%omegar)))/(2._dl* self%gammaA)
